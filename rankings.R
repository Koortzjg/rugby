...
fread("rugby.csv") |>
    ...
    DT(is.element(home_team, fread("irb_countries.csv")[["country_name"]])) |>
    DT(is.element(away_team, fread("irb_countries.csv")[["country_name"]])) |>
    {
        \(x) {
            x |>
                DT(, .(
    ...
    DT(
        order(period_monday, -team_ranking),
        .(period_monday, team_name, team_ranking)
    ) |>
