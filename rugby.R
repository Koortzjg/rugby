...
get_espn_rugby <- function(date) {
    ...
    competition <- sprintf("//*[@id='events']/div[%d]/a", n_x) |>
    ...
    while (!is.na(competition)) {
        ...
        match <- sprintf("//*[@id='events']/div[%d]/article[%s]", n_x, n_y) |>
            html_element(page, xpath = _) |>
            html_text2()
    ...
    fread("rugby.csv") |>
        ...
        DT(,
            `:=`(
                ...
                away_conversion = fifelse(
                    is.na(away_conversion),
                    decompose_score(
                        away_score,
                        "conversions"
                    ),
        ...
        DT(, `:=`(
            home_kick = fcoalesce(home_kick, 0),
            away_kick = fcoalesce(away_kick, 0)
        )) |>
        fwrite("rugby.csv")
}
