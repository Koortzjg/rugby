...
# quarter finals and above
match_data <- fread("rugby.csv") |>
    DT(competition_name == "Rugby World Cup") |>
    DT(
        order(format(match_date, "%Y"), -match_date),
        row_number := seq_len(.N),
        by = .(year = format(match_date, "%Y"))
    ) |>
    DT(row_number <= 8, ) |>
    DT(order(match_date, row_number)) |>
    DT(, row_number := NULL) |>
    DT(, `:=`(
...
plot(
    ...
    main = "Rugby World Cup Quarter Finals and Above Matches"
)
